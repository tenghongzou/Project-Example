...
- task queue 由 `TEMPORAL_TASK_QUEUE` 環境變數設定（預設 `default`），
  scheduler 與 temporal-worker 讀同一個值，改名時兩邊自動一致
...
- 活動取消（`EventCancelled`）會自動撤銷尚未送出的提醒（cancel by workflow id）
- dev 用 `temporalio/auto-setup`（schema 建在共用 PG 的 `temporal` 資料庫）；
  正式環境建議獨立部署 Temporal cluster 或用 Temporal Cloud

已知限制（後續工作）：
- Temporal server 短暫不可用時，排程訊息走 messenger 全域 retry（約 7 秒內 3 次）
  後進 failed transport，需 `messenger:failed:retry` 重放——維運上要注意
- `temporal-worker` 目前只有 dev 建置目標（code volume 掛載）；prod 部署需要
  另建含 rr binary + code 的 production stage
...
EventManage 與 FlowEngine 的整合事件經 Redis 送出，`Notification` 模組的 handler
訂閱消費：每則通知先以 **pending** 落地 PostgreSQL（schema: `notification`），
再經 `NotificationChannel` 投遞並標記 **sent / failed**——投遞結果以資料庫紀錄為
唯一事實來源，失敗原因存在 `error` 欄位。

| Method | Path | 說明 |
|---|---|---|
| GET | `/api/notifications` | 通知列表（由新到舊，含投遞狀態） |
| GET | `/api/notifications/{id}` | 單筆（channel / status / error / sent_at） |

預設管道為 `log`（輸出見 worker log，log 內 `trace_id` 可在 Jaeger 對照完整鏈路，
context 的 `severity` 鍵決定 log 等級）；實作 `NotificationChannel` 介面即自動註冊
新管道（email/SMS/push，tagged service），不需改動既有程式。
